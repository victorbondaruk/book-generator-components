...
interface NodeFields extends Mem0MemoryInput, Mem0MemoryExtendedInput, BufferMemoryExtendedInput {
    searchOnly: boolean
    useBookGeneratorChatId: boolean
    input: string
}
        ...
        this.inputs = [
            {
                ...
                name: 'user_id',
                type: 'string',
                description: 'Unique identifier for the user. Required only if "Use BookGenerator Chat ID" is OFF.',
                default: 'book-generator-default-user',
                optional: true
            },
            // Added toggle to use BookGenerator chat ID
            {
                label: 'Use BookGenerator Chat ID',
                name: 'useBookGeneratorChatId',
                type: 'boolean',
                description: 'Use the BookGenerator internal Chat ID as the Mem0 User ID, overriding the "User ID" field above.',
                default: false,
                optional: true
            },
            {
...
const initializeMem0 = async (nodeData: INodeData, input: string, options: ICommonObject): Promise<BaseMem0Memory> => {
    ...
    const useBookGeneratorChatId = nodeData.inputs?.useBookGeneratorChatId as boolean
    const orgId = options.orgId as string

    if (!useBookGeneratorChatId && !initialUserId) {
        throw new Error('User ID field cannot be empty when "Use BookGenerator Chat ID" is OFF.')
    }
    ...
    const constructorSessionId = initialUserId || (useBookGeneratorChatId ? 'book-generator-chat-id-placeholder' : '')
    ...
    const obj: NodeFields = {
        ...
        useBookGeneratorChatId: useBookGeneratorChatId,
        input: input,
        orgId: orgId
    }
...
class Mem0MemoryExtended extends BaseMem0Memory implements MemoryMethods {
    ...
    constructor(fields: NodeFields) {
        ...
        this.useBookGeneratorChatId = fields.useBookGeneratorChatId
        this.input = fields.input
        this.orgId = fields.orgId
    }

    // Selects Mem0 user_id based on toggle state (BookGenerator chat ID or input field)
    private getEffectiveUserId(overrideUserId?: string): string {
        let effectiveUserId: string | undefined

        if (this.useBookGeneratorChatId) {
            if (overrideUserId) {
                effectiveUserId = overrideUserId
            } else {
                throw new Error('Mem0: "Use BookGenerator Chat ID" is ON, but no runtime chat ID (overrideUserId) was provided.')
            }
        } else {
    ...
    async getChatMessages(
        ...
        const bookGeneratorSessionId = overrideUserId
        if (!bookGeneratorSessionId) {
            console.warn('Mem0: getChatMessages called without overrideUserId (BookGenerator Session ID). Cannot fetch DB messages.')
            return []
        }

        let chatMessage = await this.appDataSource.getRepository(this.databaseEntities['ChatMessage']).find({
            where: {
                sessionId: bookGeneratorSessionId,
    ...
    async clearChatMessages(overrideUserId = ''): Promise<void> {
        ...
        if (bookGeneratorSessionId) {
            await this.appDataSource
                ...
                .delete({ sessionId: bookGeneratorSessionId, chatflowid: this.chatflowid })
        } else {
            console.warn('Mem0: clearChatMessages called without overrideUserId (BookGenerator Session ID). Cannot clear DB messages.')
        }
    }
}
