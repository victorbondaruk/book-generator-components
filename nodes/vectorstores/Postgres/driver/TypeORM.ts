...
import { BOOK_GENERATOR_CHATID, ICommonObject } from '../../../../src'
...
export class TypeORMDriver extends VectorStoreDriver {
    ...
    static similaritySearchVectorWithScore = async (
        ...
        let chatflowOr = ''
        const { [BOOK_GENERATOR_CHATID]: chatId, ...restFilters } = filter || {}
        ...
        const queryString = `
            ...
            FROM ${tableName}
            WHERE ((metadata @> $2) AND NOT (metadata ? '${BOOK_GENERATOR_CHATID}')) ${chatflowOr}
