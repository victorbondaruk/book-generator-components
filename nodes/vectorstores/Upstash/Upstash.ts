...
class Upstash_VectorStores implements INode {
    ...
    async init(nodeData: INodeData, _: string, options: ICommonObject): Promise<any> {
        ...
        if (isFileUploadEnabled && options.chatId) {
            if (upstashMetadataFilter) obj.filter += ` OR ${BOOK_GENERATOR_CHATID} = "${options.chatId}" OR HAS NOT FIELD ${BOOK_GENERATOR_CHATID}`
            else obj.filter = `${BOOK_GENERATOR_CHATID} = "${options.chatId}" OR HAS NOT FIELD ${BOOK_GENERATOR_CHATID}`
        }
