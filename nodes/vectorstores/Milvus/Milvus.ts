...
import { BOOK_GENERATOR_CHATID, getBaseClasses, getCredentialData, getCredentialParam } from '../../../src/utils'
...
class Milvus_VectorStores implements INode {
            ...
            for (let i = 0; i < flattenDocs.length; i += 1) {
                if (flattenDocs[i] && flattenDocs[i].pageContent) {
                    if (isFileUploadEnabled && options.chatId) {
                        flattenDocs[i].metadata = { ...flattenDocs[i].metadata, [BOOK_GENERATOR_CHATID]: options.chatId }
                    }
    ...
    async init(nodeData: INodeData, _: string, options: ICommonObject): Promise<any> {
        ...
        if (isFileUploadEnabled && options.chatId) {
            if (milvusFilter) milvusFilter += ` OR ${BOOK_GENERATOR_CHATID} == "${options.chatId}" OR NOT EXISTS(${BOOK_GENERATOR_CHATID})`
            else milvusFilter = `${BOOK_GENERATOR_CHATID} == "${options.chatId}" OR NOT EXISTS(${BOOK_GENERATOR_CHATID})`
        }
...
const similaritySearchVectorWithScore = async (query: number[], k: number, vectorStore: Milvus, milvusFilter?: string, filter?: string) => {
    ...
    searchResp.results.forEach((result) => {
        ...
        let normalizedScore = (result as any).score
        switch (vectorStore.indexCreateParams.metric_type) {
            case MetricType.L2:
                normalizedScore = 1 / (1 + (result as any).score)
                break
            case MetricType.IP:
            case MetricType.COSINE:
                normalizedScore = ((result as any).score + 1) / 2
                break
        }
