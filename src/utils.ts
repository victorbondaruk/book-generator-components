...
import { NodeVM } from 'book-generator-nodevm'
...
export const BOOK_GENERATOR_CHATID = 'book-generator_chatId'
...
/*
 * List of dependencies allowed to be import in book-generator-nodevm
 */
...
const getEncryptionKeyFilePath = (): string => {
    const checkPaths = [
        ...
        path.join(__dirname, '..', '..', '..', '..', '..', 'encryption.key'),
        path.join(__dirname, '..', '..', '..', '..', '..', 'server', 'encryption.key'),
        path.join(getUserHome(), '.book-generator', 'encryption.key')
    ]
    for (const checkPath of checkPaths) {
        if (fs.existsSync(checkPath)) {
            return checkPath
...
const getEncryptionKey = async (): Promise<string> => {
    if (process.env.BOOK_GENERATOR_SECRETKEY_OVERWRITE !== undefined && process.env.BOOK_GENERATOR_SECRETKEY_OVERWRITE !== '') {
        return process.env.BOOK_GENERATOR_SECRETKEY_OVERWRITE
    }
    try {
        if (USE_AWS_SECRETS_MANAGER && secretsManagerClient) {
            const secretId = process.env.SECRETKEY_AWS_NAME || 'BookGeneratorEncryptionKey'
...
const decryptCredentialData = async (encryptedData: string): Promise<ICommonObject> => {
    ...
    if (USE_AWS_SECRETS_MANAGER && secretsManagerClient) {
        try {
            if (encryptedData.startsWith('BookGeneratorCredential_')) {
...
const jsonEscapeCharacters = [
    { escape: '"', value: 'BOOK_GENERATOR_DOUBLE_QUOTE' },
    { escape: '\n', value: 'BOOK_GENERATOR_NEWLINE' },
    { escape: '\b', value: 'BOOK_GENERATOR_BACKSPACE' },
    { escape: '\f', value: 'BOOK_GENERATOR_FORM_FEED' },
    { escape: '\r', value: 'BOOK_GENERATOR_CARRIAGE_RETURN' },
    { escape: '\t', value: 'BOOK_GENERATOR_TAB' },
    { escape: '\\', value: 'BOOK_GENERATOR_BACKSLASH' }
]
