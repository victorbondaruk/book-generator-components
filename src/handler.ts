...
class ExtendedLunaryHandler extends LunaryHandler {
    ...
    constructor({ bookGeneratorOptions, ...options }: any) {
        super(options)
        this.appDataSource = bookGeneratorOptions.appDataSource
        this.databaseEntities = bookGeneratorOptions.databaseEntities
        this.chatId = bookGeneratorOptions.chatId
        this.apiMessageId = bookGeneratorOptions.apiMessageId
    }
            ...
            if (providerStatus) {
                ...
                if (provider === 'langSmith') {
                    ...
                    let lunaryFields = {
                        ...
                        runtime: 'book-generator',
                        bookGeneratorOptions: options
                    }
...
export class AnalyticHandler {
    ...
    async onChainStart(name: string, input: string, parentIds?: ICommonObject) {
        ...
        if (Object.prototype.hasOwnProperty.call(this.handlers, 'arize')) {
            ...
            if (!parentIds || !Object.keys(parentIds).length) {
                rootSpan = tracer ? tracer.startSpan('BookGenerator') : undefined
                if (rootSpan) {
